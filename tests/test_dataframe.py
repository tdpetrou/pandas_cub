...
class TestSelection:
    ...
    def test_multiple_int_selection(self):
    ...
    def test_col_simultaneous_slice(self):
    ...
    def test_simultaneous_select(self):
        df_result = df[2, ['a', 'e']]
    ...
    def test_boolean_select(self):
        df_result = df[df['e'] > 1]
        df_answer = pdc.DataFrame({'a': a[1:], 'b': b[1:], 'c': c[1:], 'd': d[1:], 'e': e[1:]})
        assert_df_equals(df_result, df_answer)

    def test_boolean_col_select(self):
        df_result = df[df['e'] > 1, ['b', -1]]
        df_answer = pdc.DataFrame({'b': b[1:], 'e': e[1:]})
        assert_df_equals(df_result, df_answer)
...
df_emp = pdc.read_csv('notebooks/data/employee.csv')
