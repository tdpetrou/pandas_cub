...
class DataFrame:
    ...
    def __init__(self, values):
        ...
        # new_values will be the final dictionary used to hold the data
        # after converting any unicode arrays to 'object' data type arrays
        new_values = {}
        ...
        for i, (col_name, col_values) in enumerate(values.items()):
            ...
            if col_values.dtype.kind == 'U':
                new_values[col_name] = col_values.astype('O')
            else:
                new_values[col_name] = col_values

            if length != len(col_values):
                raise ValueError('All values must be the same length')

            # map the column name to its single char 'kind'
            self._column_info[col_name] = new_values[col_name].dtype.kind

        # store values as a private variable
